...
def remove_from_order(parameters: dict, session_id: str):
    ...
    if len(current_order.keys()) == 0:
        fulfillment_text = f"Your order is empty. What would you like to order?"
    else:
...
def complete_order(parameters: dict, session_id: str):
    if session_id not in inprogress_orders:
        ...
        if order_id == -1:
            fulfillment_text = "Sorry! There was an error placing your order. Please try again later."
        else:
            order_total = DB_Helper.get_total_order_price(order_id)
            fulfillment_text = f"Your order has been placed successfully. Your order ID is {order_id}. Your order total is {order_total} which you can pay at the time of delivery."
...
def add_to_order(parameters: dict, session_id: str):
    ...
    if len(food_items) != len(quantities):
        ...
        fulfillment_text = f"So far you have : {order_str}. Anything else you would like to add?"
